(function() {
    ...
    let cachedFavs: any[] | null = null;

    function getFavs(): any[] {
        if (cachedFavs) return cachedFavs;
        try { cachedFavs = JSON.parse(localStorage.getItem(STORAGE_KEY) || '[]'); } 
        catch { cachedFavs = []; }
        return cachedFavs;
    }

    function saveFavs(favs: any[]) {
        cachedFavs = favs; 
    ...
    function processGarageMissions(garageCards: HTMLElement[]) {
        ...
        garageCards.forEach(card => {
            ...
            if (!starContainer) {
                ...
                const img = starContainer.querySelector('img') as HTMLImageElement;
    ...
    function updateInterface() {
        ...
        const cards = document.querySelectorAll('.MainQuestComponentStyle-cardPlayCommon, .TableMainQuestComponentStyle-commonTableMainQuest, .MainQuestComponentStyle-cardPlay');
        if (cards.length > 0) {
            processGarageMissions(Array.from(cards) as HTMLElement[]);
        }
    }
    ...
    const initObserver = () => {
        const rootContainer = document.body || document.documentElement;
    ...
    function gameLoop(timestamp: number) {
        ...
        if (timestamp - lastCheckTime < 1000) return;
