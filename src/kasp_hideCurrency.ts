(function () {
    ...
    function getCurrencyStyle(span: HTMLElement): string {
        if (span.closest('.ksc-22')) return 'color: rgb(255, 102, 102);';
        if (span.closest('.ksc-24')) return 'color: rgb(0, 215, 255);';
        if (span.closest('.UserScoreComponentStyle-coinBlock')) return 'color: rgb(255, 188, 9);';
        return 'color: #fff;';
    }
    ...
    function processSpan(span: HTMLElement) {
        ...
        if (!span.classList.contains('currency-masked')) {
            ...
            if (parentElement && !parentElement.dataset.tooltipAttached) {
                ...
                parentElement.addEventListener('mouseenter', (e: MouseEvent) => {
    ...
    const observer = new MutationObserver(() => {
        ...
        spans.forEach(s => processSpan(s as HTMLElement));
    ...
    if (document.body || document.getElementById('app-root')) initObserver();
