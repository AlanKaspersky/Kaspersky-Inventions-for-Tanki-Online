(function() {
    ...
    const getCurrentNickname = (): string => {
        const userEl = document.querySelector('.UserInfoContainerStyle-userNameRank') as HTMLElement | null;
    ...
    const getCustomCategories = (): Record<string, string> => {
    ...
    const setCustomCategory = (friendNickname: string, colorType: string) => {
        ...
        document.querySelectorAll('.custom-friends-sidebar').forEach(node => {
            ...
            const activeBtn = sidebar.querySelector('.custom-filter-btn.active') as HTMLElement | null;
            if (activeBtn) activeBtn.click();
        });
    };

    const getMyClanTag = (): string => {
        const userEl = (document.querySelector('.UserInfoContainerStyle-userNameRank.UserInfoContainerStyle-textDecoration') || document.querySelector('.UserInfoContainerStyle-userNameRank')) as HTMLElement | null;
        if (!userEl) return "";
        const text = userEl.innerText.trim();
    ...
    const updateCardBadge = (el: HTMLElement, isFriendsList: boolean) => {
        ...
        const span = Array.from(el.querySelectorAll('span')).find(s => s.className.includes('whiteSpaceNoWrap')) as HTMLElement | undefined;
        ...
        let rarityType: string | null = null;
        if (customColor) {
        ...
        let badge = el.querySelector('.custom-rarity-badge') as HTMLImageElement | null;
        if (rarityType) {
            if (!badge) {
    ...
    const applyFilter = (scrollBlock: HTMLElement, filterType: string) => {
        ...
        items.forEach(node => {
            ...
            if (filterType === 'all') { el.style.display = ''; return; }
    ...
    const injectCategoriesMenu = (menu: HTMLElement) => {
        ...
        const rankItem = menu.querySelector('.ContextMenuStyle-menuItemRank') as HTMLElement | null;
        if (!rankItem) return;

        const span = Array.from(rankItem.querySelectorAll('span')).find(s => s.className.includes('whiteSpaceNoWrap')) as HTMLElement | undefined;
        if (!span) return;
    ...
    const setupSidebar = (scrollBlock: HTMLElement) => {
        ...
        if (isFriends) {
            ...
            const parent = scrollBlock.parentNode as HTMLElement | null;
            if (!parent) return;
