(function() {
    ...
    const ROWS_PER_PAGE: number = 25;

    const updateNickname = () => {
        const nameEl = document.querySelector('.UserInfoContainerStyle-userNameRank') as HTMLElement | null;
        if (nameEl) {
    ...
    const addBattle = async (battleData: any): Promise<any> => {
    ...
    function showClearConfirmDialog(callback?: () => void) {
    ...
    const parseMapAndMode = (rawMapText: string) => {
        ...
        if (!cleanMapName) cleanMapName = 'Unknown';

        return { map: cleanMapName, mode: foundMode };
    };

    const extractBackgroundUrl = (element: Element | null): string => {
        if (!element) return '';
    ...
    const extractEquipmentAndAugments = (selfRow: HTMLElement) => {
        const getBg = (selector: string) => extractBackgroundUrl(selfRow.querySelector(selector));
        return {
    ...
    const renderBattleTable = async (page: number = 1) => {
        updateNickname();
        ...
        const prevBtn = document.getElementById('bh-prev-page') as HTMLButtonElement | null;
        if (prevBtn) prevBtn.disabled = currentPage === 1;

        const nextBtn = document.getElementById('bh-next-page') as HTMLButtonElement | null;
    ...
    const importHistoryData = () => {
        ...
        input.onchange = (e: Event) => {
            ...
            reader.onload = async (ev: ProgressEvent<FileReader>) => {
                try {
                    const targetResult = ev.target?.result as string;
    ...
    const injectMenuButton = () => {
        ...
        if (!menuList) return;
        if (menuList.querySelector('.custom-history-button')) return;
