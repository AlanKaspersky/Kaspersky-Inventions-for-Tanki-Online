(function() {
    ...
    let settingsWereOpen: boolean = false;
    ...
    const t: Record<string, { title: string, tooltip: string }> = {
    ...
    function injectSettingsTab() {
        const mainBlock = document.querySelector('.SettingsComponentStyle-blockContentOptions') as HTMLElement;
        if (!mainBlock) return;

        const ulMenu = mainBlock.querySelector('ul') as HTMLUListElement;
        if (!ulMenu || document.getElementById('kaspersky-tab')) return;

        const lang = getLang();
        const dict = t[lang];

        let tooltip = document.getElementById('kaspersky-reload-tooltip') as HTMLElement;
        if (!tooltip) {
        ...
        MY_SETTINGS.forEach(setting => {
            ...
            const localizedLabel = (setting.label as Record<string, string>)[lang] || (setting.label as Record<string, string>)['EN'];
            togglesHTML += `
        ...
        kTab.addEventListener('click', (e: MouseEvent) => {
            e.stopPropagation(); 
        ...
        ulMenu.addEventListener('click', (e: MouseEvent) => {
            const target = e.target as HTMLElement;
            const clickedTab = target.closest('.SettingsMenuComponentStyle-menuItemOptions') as HTMLElement;
