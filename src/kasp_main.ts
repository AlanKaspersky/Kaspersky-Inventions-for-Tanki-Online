(function () {
    ...
    const readyPromise = (async () => {
        try {
            ...
            if (!paintsRes.ok) throw new Error('paints.json: HTTP ' + paintsRes.status);
            if (!augmentsRes.ok) throw new Error('augments.json: HTTP ' + augmentsRes.status);
    ...
    const modules = {
        ...
        changeCounter: (() => {
            ...
            /** SYNC: создать `<th>` / `<td>` если их нет. Без чтения данных игроков. */
            function sync(): void {
            ...
            function update(): void {
                ...
                try {
                    ...
                    cells.forEach(cell => {
                        ...
                        let changeTd = row.querySelector('.kasp-change-td') as HTMLElement | null;
                        if (!changeTd) return;
        ...
        zeroResists: (() => {
            ...
            function getIconUrl(lbl: HTMLElement): string {
            ...
            function sync(): void {
                const theadRows = document.querySelectorAll(
                    ':is(.BattleTabStatisticComponentStyle-containerInsideTeams, .BattleTabStatisticComponentStyle-containerInsideResults) table thead tr'
                );
                theadRows.forEach(row => {
                    if (row.querySelector('.kasp-defence-th')) return;
                    const gsHeader = row.children[1];
                ...
                for (let i = 0; i < cells.length; i++) {
                    ...
                    if (cell.querySelector('.kasp-compact-cell')) continue;
