    ...
    const modules = {
        battleHistory: (() => {
            ...
            const updateNickname = () => {
                const nameEl = document.querySelector('.UserInfoContainerStyle-userNameRank') as HTMLElement;
                if (nameEl) {
                    const text = nameEl.textContent?.trim() || '';
                    const cleanName = text.replace(/^\[.*?\]\s*/, '').trim();
                    if (cleanName) currentNickname = cleanName;
                }
            };
            ...
            const translateMapName = (rawMapWithMode: string, targetLang: string) => {
                let cleanText = rawMapWithMode.trim();
                if (targetLang === 'EN') {
                    if (mapTranslations[cleanText]) return mapTranslations[cleanText];
                } else {
                    const invertedMap = Object.fromEntries(Object.entries(mapTranslations).map(([k, v]) => [v, k]));
                    if (invertedMap[cleanText]) return invertedMap[invertedMap[cleanText]];
                }
                return cleanText;
            };

            function showClearConfirmModal(onConfirm: () => void) {
                ...
                confirmBtn?.addEventListener('click', (e) => { e.stopPropagation(); if (!isClosing) { isClosing = true; closeDialog(); onConfirm(); } });
                cancelBtn?.addEventListener('click', (e) => { e.stopPropagation(); if (!isClosing) { isClosing = true; closeDialog(); } });
                closeBtn?.addEventListener('click', (e) => { e.stopPropagation(); if (!isClosing) { isClosing = true; closeDialog(); } });
                overlay.addEventListener('click', (e) => { if (e.target === overlay && !isClosing) { isClosing = true; closeDialog(); } });
            ...
            const extractBackgroundUrl = (element: HTMLElement | null) => {
                if (!element) return '';
            ...
            const extractEquipmentAndAugments = (selfRow: HTMLElement) => {
                const getBg = (selector: string) => extractBackgroundUrl(selfRow.querySelector(selector) as HTMLElement);
                return {
            ...
            const renderBattleTable = async (page = 1) => {
                ...
                pageBattles.forEach(b => {
                    ...
                    if (isDM) { statusClass = 'bh-status-dm'; statusLocalized = dict.dm; }
                    else if (isWin) { statusClass = 'bh-status-victory'; statusLocalized = dict.win; }
                    else if (isDraw) { statusClass = 'bh-status-draw'; statusLocalized = dict.draw; }
                ...
                const prevBtn = document.getElementById('bh-prev-page') as HTMLButtonElement;
                if (prevBtn) prevBtn.disabled = currentPage === 1;
                const nextBtn = document.getElementById('bh-next-page') as HTMLButtonElement;
            ...
            const injectMenuButton = () => {
                ...
                if (!menuList || menuList.querySelector('.custom-history-button')) return;
                const lang = state.lang;
                const dict = t[lang] || t['EN'];
            ...
            const extractAndSaveBattleResult = async () => {
                ...
                if (!equipCell && !selfRow.dataset.equipTimeout) return;
